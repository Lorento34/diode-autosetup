...
# ----------------------------------------------------------------------------
# Setup script for Diode HTTP Gateway Publisher with Nginx, systemd, and Diode CLI
# ----------------------------------------------------------------------------

# Elevate to root if needed
if [ "$(id -u)" -ne 0 ]; then
  echo "ℹ️ Root yetkisi gerekiyor, sudo ile yeniden çalıştırılıyor..."
  exec sudo bash "$0" "$@"
fi
...
# Log function\log() {
  echo "[$(date +'%Y-%m-%d %H:%M:%S')] $*" | tee -a /var/log/diode-autosetup.log >/dev/null
}
...
DIODE_USER="diode"
EOF
  print_info "Config oluşturuldu: $ENV_FILE"
fi

# Load config
source "$ENV_FILE"

# Create system user for Diode
...
# Obtain TLS if DOMAIN is set
if [[ -n "$DOMAIN" ]]; then
  print_info "TLS sertifikası alınıyor: $DOMAIN"
  certbot --nginx -d "$DOMAIN" --non-interactive --agree-tos -m admin@$DOMAIN
  systemctl reload nginx
  print_success "TLS sertifikası alındı"
fi

# Install Diode CLI\mkdir -p "$CLI_DIR"
chown "$DIODE_USER":"$DIODE_USER" "$CLI_DIR"
print_info "Diode CLI indiriliyor..."
...
rm "$tmp"
print_success "Diode CLI yüklendi"
